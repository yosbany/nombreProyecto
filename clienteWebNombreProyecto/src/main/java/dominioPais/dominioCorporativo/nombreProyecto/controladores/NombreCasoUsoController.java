...
public class NombreCasoUsoController extends ControladorBase {
	...
	public ModelAndView nombreOperacionInsertAndEdit(@Validated NombreFormularioForm nombreFormularioForm, BindingResult result){
		...
		objetoEntidad.setCampo(nombreFormularioForm.getCampo());		
		//Llamo a la fachada tantas veces se requiera;
		this.facade.metodo(objetoEntidad);
		
		return new ModelAndView('voy a la siguiente página');
	}

	@RequestMapping(method = RequestMethod.GET value = 'nombreOperacionList.do')
	protected AlumnoForm nombreOperacionList() throws ServletException {
	...
	protected AlumnoForm nombreOperacionDetails() throws ServletException {
		return new NombreFormularioForm();
	...
	public ModelAndView Exception(Exception e , HttpServletRequest request){		
		...
		return new ModelAndView("pagina de error");
	}*/
}
